...
class PositionControllerIntegrationTest {
    ...
    @Test
    @DisplayName("GET /api/account/accounts/{id}/positions returns positions for authenticated client's account")
    void testGetPositionsForOwnedAccount_Success() throws Exception {
        mockMvc.perform(get("/api/account/accounts/{accountId}/positions", ACCOUNT_OWNER_USD_ID)
    ...
    @Test
    @DisplayName("GET /api/account/accounts/{id}/positions returns 404 for unowned account")
    void testGetPositionsForUnownedAccount_NotFound() throws Exception {
        mockMvc.perform(get("/api/account/accounts/{accountId}/positions", ACCOUNT_OTHER_USD_ID)
                        .header("Authorization", "Bearer " + ownerToken))
                .andExpect(status().isNotFound());
    }

    @Test
    @DisplayName("GET /api/account/accounts/{id}/positions returns 401 for unauthenticated request")
    ...
    @Test
    @DisplayName("GET /api/account/accounts returns active accounts for authenticated client")
    void testGetAccounts_Success() throws Exception {
        mockMvc.perform(get("/api/account/accounts")
                        .header("Authorization", "Bearer " + ownerToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasSize(1)))
                .andExpect(jsonPath("$[0].accountId", is(ACCOUNT_OWNER_USD_ID)));
    }
}
