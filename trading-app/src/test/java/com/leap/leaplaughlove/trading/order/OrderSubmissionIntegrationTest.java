...
class OrderSubmissionIntegrationTest {
    ...
    void setUp() {
        ...
        // Seed initial cash deposit of $10,000 for the account
        cashLedgerRepository.save(new CashLedgerEntry(
                accountOwnerId, "DEPOSIT", new BigDecimal("10000.00"), "USD",
    ...
    @Test
    @DisplayName("End-to-End BUY order: fills, creates audit execution, settles cash ledger, records position movement, and increases position")
    void testEndToEndBuyOrder_Success() throws Exception {
        ...
        Order savedOrder = orders.stream()
                .filter(o -> o.getAccount().getAccountId().equals(accountOwnerId))
                .findFirst()
                .orElseThrow();
        ...
        // 3. Verify Cash Ledger in DB (-$1500.00)
        BigDecimal currentCash = cashLedgerRepository.sumAmountByAccountIdAndCurrency(accountOwnerId, "USD");
        assertEquals(0, new BigDecimal("8500.00").compareTo(currentCash));

        // 4. Verify Position Movement in DB (+10 shares)
        ...
        // 5. Verify Position in DB (initial was 25, now 35)
        Position position = positionRepository.findById(new PositionId(accountOwnerId, instrumentAaplId)).orElseThrow();
        assertEquals(35L, position.getQuantity());
    }

    @Test
    @DisplayName("End-to-End SELL order: fills, credits cash ledger, records position movement, and reduces holdings")
    void testEndToEndSellOrder_Success() throws Exception {
        // Initial holdings: 25 AAPL
        OrderSubmissionRequest request = new OrderSubmissionRequest(
        ...
        mockMvc.perform(post("/api/trading/orders")
                ...
                .andExpect(jsonPath("$.execution.status").value("FILLED"))
                .andExpect(jsonPath("$.accountBalanceAfter").value(11000.00)); // 10000 + (5 * 200)

        // Holdings reduced from 25 to 20
        Position position = positionRepository.findById(new PositionId(accountOwnerId, instrumentAaplId)).orElseThrow();
        assertEquals(20L, position.getQuantity());
    }

    @Test
    @DisplayName("End-to-End Rejected BUY order (insufficient funds): stores REJECTED order & execution, preserves cash and positions")
    void testEndToEndBuyOrder_RejectedInsufficientFunds() throws Exception {
        ...
        // Cash unchanged at 10,000
        BigDecimal currentCash = cashLedgerRepository.sumAmountByAccountIdAndCurrency(accountOwnerId, "USD");
        assertEquals(0, new BigDecimal("10000.00").compareTo(currentCash));

        // Position unchanged at 25
        Position position = positionRepository.findById(new PositionId(accountOwnerId, instrumentAaplId)).orElseThrow();
        assertEquals(25L, position.getQuantity());
    }

    @Test
    @DisplayName("End-to-End Rejected SELL order (insufficient holdings): stores REJECTED order & execution, preserves positions")
    ...
    void testSubmitOrder_UnownedAccount_Returns404() throws Exception {
        OrderSubmissionRequest request = new OrderSubmissionRequest(
                UUID.fromString(ACCOUNT_OTHER_USD_ID), "AAPL", null, Order.Side.BUY, 10, new BigDecimal("150.00"));
