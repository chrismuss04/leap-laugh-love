...
class OrderSubmissionControllerTest {
    ...
    @Test
    @DisplayName("POST /api/trading/orders - 200 with FILLED response when valid request")
    ...
    @Test
    @DisplayName("POST /api/trading/orders/accounts/{accountId} - 200 via scoped path")
    void testSubmitOrderScopedAccount_Success() throws Exception {
        UUID clientId = UUID.randomUUID();
        UUID accountId = UUID.randomUUID();
        UUID orderId = UUID.randomUUID();
        OffsetDateTime now = OffsetDateTime.now();
        ...
        OrderSubmissionResponse.ExecutionDto execDto = new OrderSubmissionResponse.ExecutionDto(
                UUID.randomUUID(), 5L, new BigDecimal("380.00"), "FILLED", now, "Executed at market price");

        OrderSubmissionResponse mockResponse = new OrderSubmissionResponse(
                orderId, accountId, "ACC-01", UUID.randomUUID(), "MSFT", "BUY", 5L, "FILLED",
                now, now, null, now, null, execDto, new BigDecimal("7000.00"));

        when(orderSubmissionService.submitOrder(any(OrderSubmissionRequest.class))).thenReturn(mockResponse);

        OrderSubmissionRequest request = new OrderSubmissionRequest(
                accountId, "MSFT", null, Order.Side.BUY, 5, new BigDecimal("380.00"));

        mockMvc.perform(post("/api/trading/orders/accounts/{accountId}", accountId)
                        .with(csrf())
                        .with(authentication(createAuthenticationWithClientId(clientId)))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
    ...
    @Test
    @DisplayName("POST /api/trading/orders - 400 when quantity is 0")
    ...
    @Test
    @WithAnonymousUser
    @DisplayName("POST /api/trading/orders - 401 when unauthenticated")
