...
public class DatabaseInitializer {
    ...
    private void seedTestData(Statement stmt) throws SQLException {
        try {
            ...
            stmt.execute("INSERT INTO trading.orders (order_id, account_id, instrument_id, side, order_type, quantity, status, submitted_at, filled_at) " +
                    "VALUES ('" + orderIds[0] + "', '" + accountId + "', '" + aaplId + "', 'BUY', 'MARKET', 100, 'FILLED', CURRENT_TIMESTAMP - INTERVAL '2' DAY, CURRENT_TIMESTAMP - INTERVAL '2' DAY)");

            stmt.execute("INSERT INTO trading.orders (order_id, account_id, instrument_id, side, order_type, quantity, status, submitted_at, filled_at) " +
                    "VALUES ('" + orderIds[1] + "', '" + accountId + "', '" + msftId + "', 'SELL', 'MARKET', 50, 'FILLED', CURRENT_TIMESTAMP - INTERVAL '1' DAY, CURRENT_TIMESTAMP - INTERVAL '1' DAY)");

            stmt.execute("INSERT INTO trading.orders (order_id, account_id, instrument_id, side, order_type, quantity, status, submitted_at, filled_at) " +
                    "VALUES ('" + orderIds[2] + "', '" + accountId + "', '" + googlId + "', 'BUY', 'MARKET', 25, 'FILLED', CURRENT_TIMESTAMP - INTERVAL '3' HOUR, CURRENT_TIMESTAMP - INTERVAL '2' HOUR)");
